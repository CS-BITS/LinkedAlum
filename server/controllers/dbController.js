...
dbController.createUser = (req, res, next) => {
  const query = 'INSERT INTO \"user\" (user_name, password, first_name, last_name, email_address) VALUES (\'user1\', \'password\', \'Zoe\', \'Zebra\', \'zoe123@zoe.com\');'  
}
...
dbController.getAllPostsWithComments = async(req, res, next) => {
  //  Create the query statement
  try {
    const postQuery = 'SELECT post_id, message, likes, posts.created_at, "user".user_name FROM posts INNER JOIN "user" ON posts.user_id = "user".user_id ORDER BY posts.created_at DESC';
    ...
    for (let i = 0; i <  posts.length; i++) {
      ...
      try {
        const commentsQuery = 'SELECT comment_id, message, comments.created_at, "user".user_name FROM comments INNER JOIN "user" ON comments.user_id = "user".user_id WHERE post_id=' + posts[i].post_id;
...
dbController.createPost = async (req, res, next) => {
  ...
  try {
    //  Create the query statement
    const query = 'INSERT INTO "posts" (user_id, likes, message) VALUES ($1, $2, $3) RETURNING post_id;'
...
dbController.updatePostMessage = async (req, res, next) => {
  try {
    //  Create the query statement
    const query = 'UPDATE posts SET message = \'' + req.body.message + '\' WHERE post_id=' + req.body.post_id;
...
dbController.updatePostLikes = async (req, res, next) => {
  ...
  try {
    ...
    try {
      const updateLikes = 'UPDATE posts SET likes = ' + likes + ' WHERE post_id=' + req.params.id;
...
dbController.createComment = async (req, res, next) => {
  ...
  try {
    //  Create the query statement
    const query = 'INSERT INTO comments (post_id, user_id, message) VALUES ($1, $2, $3) RETURNING comment_id;'
...
dbController.updateComment = async (req, res, next) => {
  try {
    //  Create the query statement
    const query = 'UPDATE comments SET message = \'' + req.body.message + '\' WHERE comment_id=' + req.body.comment_id;
    const response = await db.query(query);
